...
import q_control_work.OperationType
import q_control_work.WorkerType
import s_abstract_classes.*
import java.io.File

class Accountant : Worker, Cleaner, Supplier {
    constructor(name: String, age: Byte) : super(name, age, WorkerType.ACCOUNTANT)
    ...
    private fun loadWorkersFromFile(): MutableList<Worker> {
        ...
        return workersFile.readLines().mapNotNull { if (it.isNotEmpty()) fromJson(it) else null }
            .toMutableList()
    }

    override fun clean() {
        println("Убираю в бухгалтерской")
    }
}